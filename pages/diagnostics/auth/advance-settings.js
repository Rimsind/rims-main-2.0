...
import { DiagnosticSideBar } from "components/common";
const AdvanceSettings = () => {
  const { auth } = useAuth();
  const { data } = useSWR(
    `${apiUrl}/diagnostic-centers/${auth?.user?.profileId}`,
    async (url) => {
  ...
  const { data: delete_request } = useSWR(
    `${apiUrl}/delete-requests?polyclinic=${auth.user?.profileId}`,
    async (url) => {
      const res = await axios.get(url, {
        headers: {
          Authorization: `Bearer ${auth.token}`,
  ...
  const submit_delete = async (data, event) => {
    ...
    try {
      const payload = {
        diagnostic: auth?.user?.profileId,
      ...
      <div className="main-wrapper">
        ...
        <div className="content">
          <div className="container-fluid">
            <div className="row">
              <DiagnosticSideBar data={data} status6="active" />
